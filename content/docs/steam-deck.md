...
Prefer the signed AppImage on SteamOS, Bazzite, and other immutable images. The AppImage bundles its own Python runtime, works without installing anything system-wide, and receives the built-in verified updater. For a reproducible install, download the v1.12.0 script, inspect it, then run it:
