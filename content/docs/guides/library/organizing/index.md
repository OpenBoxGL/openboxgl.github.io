...
`collection` is a separate grouping field populated by imports (Steam, Heroic, Lutris, Gameyfin, Arcade, Xbox 360) and shown and filtered in the native Tk UI. It feeds the **Related Games** scorer (two games with the same `collection` score higher). The Web UI does not currently expose an editable `collection` control or a `collection:` search term, so its value is set by imports and surfaced through related-game reasons rather than by hand in the browser.
