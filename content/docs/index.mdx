...
The full-featured UI ships as a native WebKitGTK window by default, with the same UI available over the loopback web server for development. Both render the identical `index.html`, `app.js`, and `app.css`, and both read and write the same local library data, so there is no second interface to drift.
