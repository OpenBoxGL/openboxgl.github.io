...
When you start the Web UI for the first time, OpenBoxGL does several things silently: it creates your data directory (`~/.local/share/openbox-game-launcher` by default), generates a per-launch `server.token`, writes the port to `server.port`, and initializes an empty `library.json` with schema version 4. You won't see these files unless you look in the data directory, but they're the reason nothing gets corrupted when the server restarts unexpectedly. See [Interfaces and data](/interfaces-and-data/) for where every file lives.
...
<li><strong>Install and start the Web UI.</strong> From the AppImage, Flatpak, or source as described on the [Installation](/install/) page. When the library is empty, the welcome wizard appears; you can close it and use the topbar buttons, or start from its **Import a ROM or game folder** step.</li>

<Callout type="note">

**You should see:** An empty cover grid in the center, no cards in any view. The welcome wizard suggests importing a folder or scanning for emulators. If the browser did not open, look at the terminal — OpenBoxGL printed `http://127.0.0.1:PORT/?token=...`. Open that URL manually.

</Callout>

<li><strong>Choose Import Folder.</strong> Click **Import Folder** in the topbar (or the wizard's folder step). The web UI asks for the absolute path of the folder to import. Because the browser cannot browse your filesystem, you type or paste the path, for example `/home/you/test-game`.</li>

<Callout type="note">

**You should see:** A text input field labeled "Folder path". Type or paste the path and press Enter. There is no file picker — you must provide the full path.
