...
<li>Resolves `openbox-release.pub` from the release and checks its SHA-256 against the pinned bootstrap anchor. `-PublicKeyPath` replaces that anchor for mirror or offline installs, and the checksum and signature gates still apply.</li>
  <li>Downloads `OpenBox-x86_64-windows.zip` with its `.sha256` and `.sig`, then fails closed if any check does not match. `-Arch` accepts `x86_64` (or `amd64`); ARM64 values fail because there is no ARM64 Windows zip.</li>
...
| `running scripts is disabled on this system` | Run the installer or launcher with `powershell -ExecutionPolicy Bypass -File .\install.ps1 -Tag v1.14.0`, or set the execution policy for your user. |
