...
The [changelog](/changelog/) is the accurate record. The current release, 1.5.1, brings performance optimizations for large library state writes, cached library projections, synchronous job lifecycle cleanup, and cross-store deduplication hardening, building on 1.5.0 which added Proton and Wine prefix discovery and management, Faugus Launcher scanning and import, Eden Nintendo Switch emulator support, canonical identity deduplication across storefronts, expanded launch command variables (`{ImagePath}`, `{dir}`, `{EmulatorDir}`, etc.), Edit Game modal next/previous navigation, platform-scoped media cleanup, play statistics reset, and smart capability playlist rules. Previous milestones include 1.4.0 (JavaScript module bundling and repository layout reorganization), 1.3.0 (bubblewrap plugin sandbox and refactored core helpers), 1.2.0 (Ed25519-signed artifacts, SteamOS startup fix, webhook/backup/7z/Gameyfin hardening), and 1.0.0 (native WebKitGTK window, batch metadata auto-match, Server-Sent Events, frozen v1 API contract, cover grouping), and releases continue shipping with complete SBOM and verification tooling.
