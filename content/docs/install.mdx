...
The fastest path is the [one-liner install](#one-liner-install): `curl ... | bash` downloads the latest AppImage, pins the release public key, verifies its SHA-256 checksum and Ed25519 signature, and installs it to `~/.local/bin`.
...
<InstallOptionCard slot="appimage">
 ...
 The script resolves the latest release automatically from GitHub, verifies the release public key against a pinned SHA-256 trust anchor, checks the AppImage against its published SHA-256, verifies its Ed25519 signature, and adds a friendly `openbox` launcher. Install to a different directory with `OPENBOX_INSTALL_DIR` (e.g. `OPENBOX_INSTALL_DIR="$HOME/Applications"`).
...
<InstallOptionCard slot="source">
 ...
 The native window opens (build it first with `make native-host`). `python3 web_app.py` starts the loopback web UI in a browser instead.
...
<li>Run the entry point from a terminal. It should open the native window showing the three-column workspace.</li>
  <li>On first launch with an empty library, the welcome wizard appears.</li>
...
Your first launch creates the data directory with an empty library. The window shows the welcome wizard when the library is empty; it walks you through a folder import, a Steam import, metadata syncing, and settings.
