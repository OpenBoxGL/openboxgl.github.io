...
The fastest path is the [versioned release installer](#versioned-release-installer): `curl ... | bash` downloads the latest AppImage, pins the release public key, verifies its SHA-256 checksum and Ed25519 signature, and installs it to `~/.local/bin`.
