...
Plugins are local Python packages that extend OpenBoxGL through six hooks. They run as separate processes with a JSON stdin/stdout protocol, so a plugin that crashes or misbehaves cannot take down the library.
...
- Hooks are `library`, `before_launch`, `after_session`, `command`, `library_source`, and `events`. Declared hooks must be a subset of these six.
...
- [Plugin hooks reference](/reference/plugins/hooks/), the hook payloads and response rules
