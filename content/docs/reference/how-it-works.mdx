...
OpenBoxGL has a local application core with one presentation layer served by two hosts. The web UI is the loopback HTTP server; on Linux the native window is a small C WebKitGTK shim that spawns that same server as a child and renders the identical `index.html`, and on Windows the WebView2 host `native_host_win.c` plays the same role (see [Windows](/windows/)). Both hosts use the same data directory and state-store rules for durable library data, and both run the same server process, so there is no second presentation stack to keep in parity.
...
AppImage or source entry point
 ...
 -> on Windows: openbox.cmd / openbox.ps1 -> native_host.exe (WebView2) or web_app.py --web
...
Launch path
 ...
 -> Popen(start_new_session=True on POSIX; creationflags on Windows)
...
The composition root is `web_app.py`, which owns the loopback server and the domain logic. The native host is `native_host.c`, a C shim that spawns `web_app.py --no-browser` and renders its `index.html` in a WebKitGTK window; Windows uses the WebView2 host `native_host_win.c`, which spawns `python.exe -B web_app.py --no-browser` and is shipped compiled as `native_host.exe` in the release zip. Shared launch and data-directory entry points live in `openbox.py`. Persistence rules are in `state_store.py`. The single renderer is `index.html`; other modules own narrower rules, such as archive safety, import adapters, metadata, saves, integrations, plugins, themes, and jobs.

## Startup and process ownership

The normal native startup path is `native_host` spawning `web_app.main()` with `--no-browser`. The web development path is `web_app.main()` directly. The shell scripts and AppImage wrapper select which host runs, but they do not become a second application layer. On Windows the launchers are `openbox.cmd`, `openbox.ps1`, and `openbox-native.ps1`, which run `native_host.exe` when it is present and otherwise fall back to the browser app window; `--web` forces the browser UI.
...
Configured startup and shutdown commands are parsed with `shlex.split()` on POSIX and MSVCRT quoting on Windows (`pkg/platform_compat.split_command()`). Their first executable path is expanded, and the command runs in a new process session on POSIX or a new process group on Windows (`launch_kwargs()`). Startup command failures are logged or ignored by the current implementation rather than becoming a different application startup mode.
...
| Native window (Windows) | `native_host.exe` from `native_host_win.c` (spawns `python.exe -B web_app.py --no-browser`) | Same chrome surface over WebView2: single instance per data directory, tray icon, geometry, native dialogs, `openbox://` deeplinks | Application logic, HTTP routes |
...
On Windows the default is `%LOCALAPPDATA%\openbox-game-launcher` (`%APPDATA%` when `LOCALAPPDATA` is unset), and `OPENBOX_DATA_DIR` overrides both platforms. The resolution lives in `pkg/platform_compat.py`; see [Windows](/windows/) for the install layout.

When the custom variable is not set and the new library file does not exist, the startup code can copy the legacy LaunchBox Linux library from its old location (`~/.local/share/launchbox-linux/library.json`, `USERPROFILE` on Windows). That migration happens before normal state-store use. It is a one-time compatibility copy, not a general synchronization mechanism.
...
The environment variable must be available before startup selects `DATA`. An `.env` file discovered later in bootstrap cannot retroactively change that already-selected data directory. Export it from the shell, desktop entry, or service that starts OpenBoxGL. On Windows set it in the process environment (for example `$env:OPENBOX_DATA_DIR` in PowerShell) before running a launcher; the launchers read it the same way.
...
`default_state()` creates schema version 6 with these top-level collections:
...
Normalization is deliberately selective. Unknown top-level and per-game fields are retained. Known fields such as IDs, queue, notifications, and tags are repaired or capped. A schema-6 file can use a fast path when its required keys and basic collection shapes already look valid. That fast path does not prove that every nested settings, playlist, queue, notification, or active-session value has valid application semantics. Feature code still validates its own input at the boundary where it uses it.
...
The sidecar lock is `fcntl.flock` on POSIX and a byte-range `msvcrt.locking` lock on Windows; both live in `pkg/platform_compat.py` and release automatically when the owning process dies.
...
mutator
 ...
 -> JSON serialized
 -> temporary primary file created beside library.json
 -> temporary file flushed, fsynced, and chmod 0600
 -> temporary backup file created and committed via os.replace(temp_bak, library.json.bak)
...
Files below the compact serialization threshold are pretty-printed. A payload whose compact form exceeds 1 MiB (or library with >500 games) is written compactly. The file ends with a newline in either case. The backup is staged into a temporary file and committed via `os.replace()` before the primary replacement, ensuring the backup write is atomic without risking corruption to the existing `.bak`. The `chmod 0600` steps are POSIX ownership hardening; on Windows `os.chmod` only toggles the read-only attribute, so the data directory's ACL is what limits access there.
...
A folder import in the native window starts with a GTK file dialog (an `IFileDialog` folder picker in the Windows host). In the browser fallback it starts with an absolute path, because the browser cannot browse the host filesystem itself:
...
| --- | --- |
| Steam | Reads VDF/manifests from known Steam roots and library folders. Launch selection falls back through native Steam, Flatpak Steam, and `xdg-open`. On Windows the roots come from the registry and `Program Files` and the launcher is `steam.exe -applaunch` (`pkg/platform_compat.steam_install_roots()`). |
| Heroic | Reads Epic, GOG, and Amazon JSON manifests. On Windows it reads them from `%APPDATA%\heroic` and launches the recorded executable directly. |
| Lutris | Invokes the native or Flatpak Lutris command with a bounded subprocess timeout and classifies sources such as Xbox, EA, Ubisoft, and generic Lutris. Lutris is a Linux launcher, so this adapter has no Windows equivalent. |
...
3. the `.sh` fallback, which invokes `bash <path>` (`sh` when `bash` is absent; on Windows, where neither is normally present, it raises "Shell scripts require bash or sh on PATH.");
4. a direct executable path when no command is configured.

Configured command text is tokenized with `shlex.split()` on POSIX and MSVCRT quoting on Windows, both through `pkg/platform_compat.split_command()`. Literal replacements then fill `{path}`, `{name}`, `{app_id}`, `{heroic_app_id}`, `{lutris_id}`, and `{rom_name}`. Because replacement occurs after splitting, a path containing spaces stays inside its existing argument. Shell operators in the configured command are not interpreted by a shell. A profile that does not contain `{path}` receives the selected launch path as a final argument.

The direct fallback requires an executable file in the Web UI launch path. On POSIX that means the execute bit; on Windows `is_executable()` accepts the executable suffixes `.exe`, `.bat`, `.cmd`, `.com`, `.lnk`, and `.ps1`. This early validation is intentional. It avoids starting a process that cannot perform useful work and gives the browser a message naming the missing command, permission, path, or dependency.
...
The Web launcher passes `start_new_session=True` to `subprocess.Popen` on POSIX, where the launched game becomes the leader of a new process session and process group. Pause, resume, stop, restart, and force-close then operate on the group with `SIGSTOP`, `SIGCONT`, `SIGTERM`, or `SIGKILL` rather than targeting only the direct child. On Windows the same call goes through `pkg/platform_compat.launch_kwargs()`, which passes `CREATE_NEW_PROCESS_GROUP` instead; pause and resume use `NtSuspendProcess`/`NtResumeProcess` on the process and its descendants, and stop or force-close uses `taskkill /PID <pid> /T` (adding `/F` for force).

Live process data is held in `RUNNING` and `PROCESSES` under `PROCESS_LOCK`, with active sessions persisted to `active_sessions` in `library.json` (Schema v6). It includes the launch ID, stable identity information, PID, process start time, selected profile, and pause state. On startup, `reconcile_sessions_on_startup()` automatically reattaches to surviving game processes and transitions deceased ones to `abandoned`. Liveness and process identity checks go through `pkg/platform_compat.process_alive()`: Windows never uses `os.kill(pid, 0)`, which CPython maps to `TerminateProcess` there, and reads start time, command line, and image name through `ctypes` instead of `/proc`.
...
There are two practical limits to remember. First, although live session records persist and reattach across normal restarts, ungraceful kernel crashes that terminate processes mark unmonitored sessions as `abandoned`. Second, a game deleted while it is running can leave a name-based history record without a current game record to receive the playtime update.
...
This boundary isolates crashes, timeouts, malformed output, and nonzero exits from the main process. It does not sandbox a trusted local plugin. The child still has the user's privileges and can read or modify files available to that account. On Linux the runner is additionally wrapped in a bubblewrap sandbox when `bwrap` is available; on Windows `bwrap` does not exist, so plugins are skipped with a warning unless `OPENBOX_ALLOW_UNSANDBOXED_PLUGINS=1`, which runs them unsandboxed as a plain subprocess. `OPENBOX_SAFE_MODE=1` skips plugin hooks and disables the webhook dispatcher, making it the first diagnostic setting for plugin-caused failures.
...
The repository's tests are standalone `test_*.py` scripts collected by `run_all_tests.sh`. On Windows there is no bash, so `python -B scripts\run_windows_tests.py` runs every `tests/test_*.py` in a subprocess with a 120-second timeout per file, and the individual `scripts/check_*.py` gates run directly; `make check` remains the full Linux gate. They use temporary directories, real subprocesses, threaded HTTP handlers, migrations, locks, archives, and integration fixtures. That makes the tests useful for tracing boundaries, not only for checking individual functions.
...
8. run `./run_all_tests.sh` and the packaging checks when the distribution path changes (`python -B scripts\run_windows_tests.py` plus the `scripts/check_*.py` gates on Windows, where bash and `make check` are not available).
