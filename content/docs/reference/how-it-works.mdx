...
| Shell | Entry point | Owns | Does not own |
| --- | --- | --- | --- |
| Web UI | `web_app.py` and `index.html` | HTTP routes, browser state, Big Box, integrations, plugin hooks, live process maps, session polling | A durable browser database or a separate library format |
| Native UI | `openbox.py` | Lightweight Tk presentation and direct launch controls | Web routes, browser rendering, web plugin flow, and Web UI live-session maps |
...
| Web runtime state | `RUNNING`, `PROCESSES`, `SESSION_EVENTS`, browser caches | One Web UI process | Python memory |
...
- [Interfaces and data](/interfaces-and-data/) for the two shells and the data-directory layout.
