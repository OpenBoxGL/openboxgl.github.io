...
```json
{
  "report": {
    ...
    "python": "3.12.3",
    "platform": "Linux-...",
    "data_dir": "~/.local/share/openbox-game-launcher",
    "games_count": 42,
    "running_count": 0,
    "settings_summary": { ... }
  }
...
Opens a native file or folder picker dialog on the host system. Body: `{"type": "file" | "folder", "prompt": "..."}`. Returns `{"path": "/selected/path"}` or `{"cancelled": true}`.

</ApiEndpoint>

<ApiEndpoint method="POST" path="/api/native/reveal">

Reveals a file or folder in the default desktop file manager (e.g. Nautilus, Dolphin). Body: `{"path": "/path/to/game.rom"}`.

</ApiEndpoint>

<ApiEndpoint method="POST" path="/api/native/open-external">

Opens an external HTTP/HTTPS URL or file path in the host's default web browser or application. Body: `{"url": "https://..."}`.

</ApiEndpoint>

<ApiEndpoint method="POST" path="/api/native/window">

Controls the native application window (fullscreen toggle, minimize, close). Body: `{"action": "fullscreen" | "minimize" | "close"}`.
