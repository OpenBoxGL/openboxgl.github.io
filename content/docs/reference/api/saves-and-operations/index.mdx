...
Save management, library backups, statistics sync, save-tool hooks, high scores, and OBS. Catalog synchronization is documented in [Statistics sync](/guides/sessions-saves-and-backups/statistics-sync/). All routes require `X-OpenBox-Token: TOKEN`; POST bodies are JSON objects. Games are located by `id` (numeric index) or `game_id` (stable id).
...
Returns `202 {"state": "queued", "job_id": "<id>"}` — backup creation runs as a durable background job. Poll the job via `GET /api/v2/jobs` (cancel with `POST /api/v2/jobs/cancel`); the worker result carries `{"archive": "<path>", "name": "<filename>"}`.
...
Downloads a redacted, portable export of the current library state as a JSON attachment (`openbox-portable-redacted-export.json`). This is a redacted export (credentials and secrets stripped), not a raw `library.json` snapshot nor a backup archive; it is useful for inspection and external tooling. The response is streamed with `Content-Disposition: attachment; filename=openbox-portable-redacted-export.json`.
...
Returns `202 {"state": "queued", "job_id": "<id>"}` — restore runs as a durable background job. Poll the job via `GET /api/v2/jobs` (cancel with `POST /api/v2/jobs/cancel`); the worker result carries `{"restored": ["library", ...]}`. Media restores bump the media epoch.
...
`{"username", "api_key"}` validates against `API_GetUserProfile.php` and persists to `retroachievements.json` (mode `0o600`). Rejected credentials raise `400 "RetroAchievements rejected those credentials."` — RetroAchievements `401`/`403` responses are translated to that message instead of a generic HTTP failure, and a profile without a `User` fails the same way. Returns the profile summary.
...
Compare the current library state against a named backup archive. Query parameter: `archive=<name>` (the backup archive name as listed by `GET /api/backups`).

Returns `{"added": [...game_ids], "removed": [...game_ids], "changed": [...game_ids], "settings_changed": <bool>, "summary": {"added": <count>, "removed": <count>, "changed": <count>, "total": <count>}}` (`total` is the current library game count).
...
Missing games return `404`; destructive operations validate prerequisites before touching files; provider and tool failures return `400` with the exact reason. An unreachable metadata database during LBDB sync fails the job with `"Could not reach the metadata database (<reason>). Check the connection and try again."` instead of a raw socket message. See [REST API overview](/reference/api/overview/) for the shared envelope and [Save archives](/reference/save-archives/) / [Library backups](/reference/library-backups/) for the archive contracts.
