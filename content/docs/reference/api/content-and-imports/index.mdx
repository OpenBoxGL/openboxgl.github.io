...
### `GET /api/faugus/status` | `GET /api/faugus/scan` | `POST /api/faugus/import`

Faugus Launcher discovery and import. `GET /api/faugus/status` returns `{"installed": bool, "data_dirs": [...]}`; `GET /api/faugus/scan` returns `{"games": [...], "count": n}`; `POST /api/faugus/import` imports scanned games with UMU launch configurations and wine prefixes.

### `GET /api/wine/prefixes` | `GET /api/wine/protons` | `GET /api/wine/prefix-for-game`

Proton and Wine runner management. `GET /api/wine/prefixes` returns `{"prefixes": [...], "available": bool}`; `GET /api/wine/protons` returns `{"protons": [...], "available": bool}`; `GET /api/wine/prefix-for-game?game_id=<id>` resolves prefix mappings per game.
...
`POST /api/v1/metadata/match` (aliased at `POST /api/metadata/match`) triggers a background job (`"metadata-match"`) to batch auto-match library titles against the local LaunchBox database for all games currently missing `launchbox_db_id`.
