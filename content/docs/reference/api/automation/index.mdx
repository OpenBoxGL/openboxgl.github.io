...
Server-Sent Events (SSE) stream endpoint for live UI and client push. Clients receive real-time notifications for:
- `ping`: Periodic keep-alive heartbeats
- `library`: Emitted when games or playlists are added, updated, or deleted
- `jobs`: Emitted when background tasks change state (e.g. metadata sync, media downloads, emulator installs)
- `media`: Emitted when media epochs increment
- `sessions`: Emitted on game launch, state updates, or session termination

Max 16 concurrent subscribers; 64 KiB buffer per connection. Requires `X-OpenBox-Token` or `?token=`.
...
`{"source", "external_id"}` removes it. Returns `{"removed": bool, "count": int}`.
