...
<Callout type="caution" title="Prefer the header over the query string">
...
| `400` | Validation error; the `error` field names the exact problem (absolute paths are sanitized: the server log keeps the full path, the client receives the prefix before the path) |
...
<FeatureGrid
 ...
 items={[
  ...
  { href: '/reference/api/saves-and-operations/', title: 'Saves and operations', eyebrow: 'protect', description: 'Save discovery/backup/restore, library backups, statistics sync, save tools, high scores, OBS, and links to catalog sync.' },
  { href: '/reference/api/one-eleven/', title: 'API 1.11 additions', eyebrow: 'release', description: 'Quick Resume, Moments, clips, Time Machine, Backlog Radio, Household, Arcade kiosk, Steam Bridge, ES-DE, SteamGridDB, and trophies.' },
  { href: '/reference/api/one-twelve/', title: 'API 1.12 additions', eyebrow: 'release', description: 'Smart collections, Game Story, per-game launch environment, scheduled backups, and the SQLite default threshold.' },
  { href: '/reference/api/one-fourteen/', title: 'API 1.14 additions', eyebrow: 'release', description: 'Plugins 2.0, Game DNA search, library health score, backlog routes, duplicate merge, missing-file repair, and effortless metadata.' },
...
Where `$TOKEN` and the port come from the running server (`cat ~/.local/share/openbox-game-launcher/server.token`). See [Configuration](/reference/configuration/) for the data directory and [the application repository](https://github.com/vindeckyy/OpenBoxGL) for the current route contract (`routes.py`).
